...
DELIMITER $$
CREATE TRIGGER `update_stats` BEFORE INSERT ON `logs` FOR EACH ROW BEGIN
    INSERT INTO `daily_logs` (
        ...
        date(NEW.`date`), 
        NEW.`demand`/60, NEW.`date`, NEW.`demand`/60, 
        NEW.`feedin`/60, NEW.`date`, NEW.`feedin`/60, 
        NEW.`consumption`/60, NEW.`date`, NEW.`consumption`/60, 
        (NEW.`pv1` + NEW.`pv2`)/60, NEW.`date`, (NEW.`pv1` + NEW.`pv2`)/60, 
        NEW.`battery_percent`, NEW.`date`, 
        NEW.`battery_percent`, NEW.`date`, 
        NEW.`temperature`, NEW.`date`, 
        NEW.`temperature`, NEW.`date`
    ) ON DUPLICATE KEY UPDATE 
        `demand_max`               = IF(NEW.`demand`/60 > IFNULL(`demand_max`, 0), NEW.`demand`/60, IFNULL(`demand_max`, 0)), 
        `demand_max_time`          = IF(NEW.`demand`/60 > IFNULL(`demand_max`, 0), NEW.`date`, IFNULL(`demand_max_time`,NEW.`date`)),
        `demand_sum`               = IFNULL(`demand_sum`,0) + NEW.`demand`/60, 
        `feedin_max`               = IF(NEW.`feedin`/60 > IFNULL(`feedin_max`, 0), NEW.`feedin`/60, IFNULL(`feedin_max`, 0)), 
        `feedin_max_time`          = IF(NEW.`feedin`/60 > IFNULL(`feedin_max`, 0), NEW.`date`, IFNULL(`feedin_max_time`,NEW.`date`)),
        `feedin_sum`               = IFNULL(`feedin_sum`,0) + NEW.`feedin`/60, 
        `consumption_max`          = IF(NEW.`consumption`/60 > IFNULL(`consumption_max`, 0), NEW.`consumption`/60, IFNULL(`consumption_max`, 0)), 
        `consumption_max_time`     = IF(NEW.`consumption`/60 > IFNULL(`consumption_max`, 0), NEW.`date`, IFNULL(`consumption_max_time`,NEW.`date`)),
        `consumption_sum`          = IFNULL(`consumption_sum`,0) + NEW.`consumption`/60, 
        `pv_max`                   = IF((NEW.`pv1` + NEW.`pv2`)/60 > IFNULL(`pv_max`, 0), (NEW.`pv1` + NEW.`pv2`)/60, IFNULL(`pv_max`, 0)), 
        `pv_max_time`              = IF((NEW.`pv1` + NEW.`pv2`)/60 > IFNULL(`pv_max`, 0), NEW.`date`, IFNULL(`pv_max_time`,NEW.`date`)),
        `pv_sum`                   = IFNULL(`pv_sum`,0) + (NEW.`pv1` + NEW.`pv2`)/60,  
    ...
    INSERT INTO `weekly_logs` (
        ...
        date_format(NEW.`date`, '%x-%v'), NEW.`demand`/60, NEW.`feedin`/60, NEW.`consumption`/60, (NEW.`pv1` + NEW.`pv2`)/60
    ) ON DUPLICATE KEY UPDATE 
        `demand`               = IFNULL(`demand`,0) + NEW.`demand`/60, 
        `feedin`               = IFNULL(`feedin`,0) + NEW.`feedin`/60, 
        `consumption`          = IFNULL(`consumption`,0) + NEW.`consumption`/60, 
        `pv`                   = IFNULL(`pv`,0) + (NEW.`pv1` + NEW.`pv2`)/60;

    INSERT INTO `monthly_logs` (
        `period`, `demand`, `feedin`, `consumption`, `pv`
    ) VALUES (
        date_format(NEW.`date`, '%Y-%m'), NEW.`demand`/60, NEW.`feedin`/60, NEW.`consumption`/60, (NEW.`pv1` + NEW.`pv2`)/60
    ) ON DUPLICATE KEY UPDATE 
        `demand`               = IFNULL(`demand`,0) + NEW.`demand`/60, 
        `feedin`               = IFNULL(`feedin`,0) + NEW.`feedin`/60, 
        `consumption`          = IFNULL(`consumption`,0) + NEW.`consumption`/60, 
    ...
    INSERT INTO `yearly_logs` (
        ...
        CAST(date_format(NEW.`date`, '%Y') AS SIGNED), NEW.`demand`/60, NEW.`feedin`/60, NEW.`consumption`/60, (NEW.`pv1` + NEW.`pv2`)/60
    ) ON DUPLICATE KEY UPDATE 
        `demand`               = IFNULL(`demand`,0) + NEW.`demand`/60, 
        `feedin`               = IFNULL(`feedin`,0) + NEW.`feedin`/60, 
        `consumption`          = IFNULL(`consumption`,0) + NEW.`consumption`/60, 
